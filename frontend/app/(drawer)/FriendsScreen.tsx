  ...
  return (
    <View className="flex-1 bg-background pt-10">
      ...
      <View className="flex-row items-center justify-center gap-x-4 px-6 mb-4">
        ...
        <TouchableOpacity
          className="w-12 h-12 rounded-full border-[3px] border-[#1BC6D9] items-center bg-[#CFF6FB] pb-[1px]"
          onPress={() => setModalVisible(true)}
        >
          <Text className="text-[28px] text-[#222] font-bold">+</Text>
        </TouchableOpacity>
      </View>
      {/* Friends List */}
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            onLongPress={() => handleRemoveFriend(item.uid)}
            className={"px-2"}
          >
